...
export default function ReporteriaPage() {
  ...
  return (
    <div className="flex flex-col gap-6 max-w-6xl mx-auto w-full pb-20 p-4">
                  ...
                  {recentList.map((item) => (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between mt-auto pt-1 border-t border-slate-50">
                        <span className="text-[9px] font-medium text-slate-400">S/ {item.monto_solicitado?.toLocaleString()}</span>
      ...
      {solicitud && (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-6 duration-700">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-2xl shadow-xl border-l-8 border-l-blue-600 print:hidden">
            <div>
              <div className="flex items-center gap-3 mb-1">
                <Badge className="bg-blue-100 text-blue-700 font-black px-3 py-1 text-xs">SOLICITUD: {solicitud.numero_solicitud}</Badge>
            ...
            <Card className="shadow-lg border-none ring-1 ring-slate-200">
              ...
              <CardContent className="p-4 space-y-4">
                <div className="space-y-2">
                   <div className="flex justify-between text-xs font-bold"><span>Total Ingresos:</span><span className="text-green-600">S/ {totalIngresos.toLocaleString()}</span></div>
                   <div className="flex justify-between text-xs font-bold"><span>Total Gastos:</span><span className="text-red-500">S/ {totalGastos.toLocaleString()}</span></div>
                   <div className="flex justify-between text-xs font-bold border-t pt-1"><span>Sobra Libre:</span><span className="text-blue-700">S/ {capacidadPrevia.toLocaleString()}</span></div>
                </div>
                <div className="p-3 bg-blue-50 rounded-xl">
                   <p className="text-[10px] font-black text-blue-600 uppercase mb-1">Monto Solicitado</p>
                   <p className="text-2xl font-black text-blue-900 tracking-tighter">S/ {(parseFloat(credito.monto_solicitado) || 0).toLocaleString()} <span className="text-sm font-medium text-slate-400">/ {credito.plazo_meses} Meses</span></p>
                </div>
              </CardContent>
            </Card>
          ...
          <div className="bg-blue-50/50 p-6 rounded-2xl border border-blue-100 flex flex-col items-center gap-4 text-center print:hidden shadow-inner">
